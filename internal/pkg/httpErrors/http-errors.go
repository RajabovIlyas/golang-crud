...
func ParseErrors(err error) RestErr {
	...
	case errors.Is(err, gorm.ErrDuplicatedKey):
		return NewRestError(http.StatusConflict, ErrEmailAlreadyExists, err)
	case errors.Is(err, sql.ErrConnDone):
		return NewRestError(http.StatusInternalServerError, ErrorConnDB.Error(), err)
...
func ErrorResponse(err error) (int, interface{}) {
	...
	return parseErrors.Status(), parseErrors
}
