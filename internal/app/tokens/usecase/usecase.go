...
type tokenUS struct {
	...
	redisRepo tokens.RedisRepository
	logger    zerolog.Logger
}

func NewTokenUseCase(cfg *config.Config, tokenRepo tokens.Repository, redisRepo tokens.RedisRepository, logger zerolog.Logger) tokens.UseCase {
...
func (t tokenUS) GenerateToken(ctx context.Context, userID uuid.UUID) (models.ResponseToken, error) {
	...
	if err != nil {
		t.logger.Error().Err(err).Msg("tokenUS.GenerateToken: create a new tokens to db")
...
func (t tokenUS) FindTokenById(ctx context.Context, tokenIDStr string) (models.Tokens, error) {
	...
	if err != nil {
		t.logger.Error().Err(err).Msgf("tokenUS.FindTokenById: find a tokens by id=%v", tokenID)
		return models.Tokens{}, err
	}
...
func (t tokenUS) FindTokenByAccessKey(ctx context.Context, accessKeyStr string) (models.Tokens, error) {
	...
	if err != nil {
		t.logger.Error().Err(err).Msgf("tokenUS.FindTokenByAccessKey: find tokens by accessKey = %v", accessKey)
	}
...
func (t tokenUS) CreateToken(ctx context.Context, userID uuid.UUID) (models.Tokens, error) {
	...
	if err != nil {
		t.logger.Error().Err(err).Msg("tokenUS.CreateToken: create a new tokens")
		return models.Tokens{}, err
	}
...
func (t tokenUS) UpdateToken(ctx context.Context, tokenID uuid.UUID) (models.Tokens, error) {
	...
	if err != nil {
		t.logger.Error().Err(err).Msgf("tokenUS.UpdateToken: update tokens by tokenID = %v", tokenID)
		return models.Tokens{}, err
	}
...
func (t tokenUS) DeleteTokenById(ctx context.Context, tokenID uuid.UUID) error {
	...
	if err != nil {
		t.logger.Error().Err(err).Msgf("tokenUS.DeleteTokenById: delete tokens by tokenID = %v", tokenID)
		return err
	}
...
func (t tokenUS) DeleteTokenByAccessKey(ctx context.Context, accessKey uuid.UUID) error {
	...
	if err != nil {
		t.logger.Error().Err(err).Msgf("tokenUS.DeleteTokenByAccessKey: delete tokens by accessKey = %v", accessKey)
		return err
	}
