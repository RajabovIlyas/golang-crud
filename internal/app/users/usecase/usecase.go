...
func NewUserUseCase(cfg *config.Config, userRepo users.Repository, redisRepo users.RedisRepository, logger zerolog.Logger) users.UseCase {
...
func (u userUC) Create(ctx context.Context, createUser models.CreateUser) (models.Users, error) {
	...
	if err != nil {
		u.logger.Error().Err(err).Msgf("userUC.Create: error create users")
		return models.Users{}, err
	}
...
func (u userUC) Update(ctx context.Context, updateUser models.UpdateUserReq) (models.Users, error) {
	...
	if err != nil {
		u.logger.Error().Err(err).Msgf("userUC.Update: error update users by id = %v", userID)
		return models.Users{}, err
	}
...
func (u userUC) Delete(ctx context.Context, userIDStr string) error {
	...
	if err != nil {
		u.logger.Error().Err(err).Msgf("userUC.Delete: error delete users by id = %v", userID)
		return err
	}
